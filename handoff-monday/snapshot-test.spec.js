/**
 * Snapshot test — guards against formatting drift in the Monday $26 menu.
 *
...
if (typeof globalThis.describe === 'function') {
  ...
  describe('Siena Monday $26 menu rendering', () => {
    // eslint-disable-next-line no-undef
...
if (process.argv[1] && process.argv[1].endsWith(fileURLToPath(import.meta.url).split('/').pop())) {
  runSnapshotTest()
    .then(() => { console.log('✓ Monday menu snapshot test passed.'); })
