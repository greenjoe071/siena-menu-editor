...
import type { MondayMenuData } from './monday-schema';
...
async function loadRenderer() {
  ...
  const mod = { exports: {} as { render?: (doc: Document, data: MondayMenuData) => void } };
  // render.js is UMD — evaluate manually, same approach as snapshot-test
  // eslint-disable-next-line no-new-func
  new Function('module', 'self', src)(mod, fakeRoot);
  const renderer = (mod.exports && mod.exports.render)
    ? mod.exports
    : (fakeRoot['SienaMondayRender'] as typeof mod.exports);
  return renderer;
}

export async function renderMondayMenu(data: MondayMenuData): Promise<string> {
