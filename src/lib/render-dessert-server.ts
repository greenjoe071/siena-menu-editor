...
import type { DessertMenuData } from './dessert-schema';
...
async function loadRenderer() {
  ...
  const mod = { exports: {} as { render?: (doc: Document, data: DessertMenuData) => void } };
  // eslint-disable-next-line no-new-func
  new Function('module', 'self', src)(mod, fakeRoot);
  const renderer = (mod.exports && mod.exports.render)
    ? mod.exports
    : (fakeRoot['SienaDessertRender'] as typeof mod.exports);
  return renderer;
}

export async function renderDessertMenu(data: DessertMenuData): Promise<string> {
